...
<script>
    ...
    async function refreshDashboard() {
        try {
            const response = await fetch(`<?php echo e(url('api/principal/dashboard')); ?>?t=${Date.now()}`, {
        ...
        try {
            const response = await fetch(`<?php echo e(url('api/principal/password-resets')); ?>?t=${Date.now()}`, {
                headers: { 'X-Requested-With': 'XMLHttpRequest' },
                cache: 'no-store'
            });
            const payload = await response.json();
